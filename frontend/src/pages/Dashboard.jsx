    ...
    <div className="min-h-screen bg-[#f8faf9] text-slate-800 font-sans pb-16">
        ...
        rightActions={
          <>
            ...
            <button
              ...
              className="bg-[#056449] hover:bg-[#04523b] text-white px-4 py-2.5 rounded-full text-xs font-extrabold flex items-center gap-1.5 transition-all duration-200 shadow-sm active:scale-[0.98] cursor-pointer"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
              </svg>
              <span className="hidden md:inline">Create New Trip</span>
            </button>
          </>
        }
      >
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-bold h-18">
          <button
            ...
            className={`h-full border-b-2 px-1 transition relative flex items-center cursor-pointer ${
              activeSubTab === 'My Trips' ? 'border-[#056449] text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
            }`}
          >
            My Trips
          </button>
          <button
            ...
            className={`h-full border-b-2 px-1 transition relative flex items-center cursor-pointer ${
              activeSubTab === 'Archive' ? 'border-[#056449] text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
            }`}
          >
            Archive
          </button>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-12">
        ...
        <div className="mb-10">
          ...
          <p className="text-sm text-slate-450 mt-1 font-semibold">Manage your upcoming adventures and past memories.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div
              onClick={() => setShowModal(true)}
              className="bg-transparent border-2 border-dashed border-slate-200 hover:border-[#056449]/40 rounded-3xl p-8 flex flex-col items-center justify-center text-center cursor-pointer min-h-[360px] group transition-all duration-200"
            >
              <div className="w-12 h-12 rounded-full bg-emerald-50 text-[#056449] flex items-center justify-center border border-emerald-100/50 mb-4 group-hover:scale-105 transition-transform duration-250">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <h3 className="font-extrabold text-slate-900 text-sm mb-1.5">Create New Trip</h3>
              <p className="text-xs text-slate-455 font-semibold max-w-[210px] leading-relaxed">
            ...
            {filteredTrips.map((trip) => {
              ...
              return (
                <div
                  ...
                  className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-[0_4px_25px_-5px_rgba(0,0,0,0.02)] hover:shadow-[0_12px_30px_-5px_rgba(0,0,0,0.04)] hover:border-[#056449]/15 cursor-pointer group transition-all duration-255 flex flex-col min-h-[360px]"
                >
                  {/* Card Banner Image */}
                  <div className="h-44 relative overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute top-4 left-4">
                      {trip.status === 'Active' && (
                        <span className="text-[10px] bg-emerald-500 text-white font-extrabold px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
                          Active
                        </span>
                      )}
                      {trip.status === 'Upcoming' && (
                        <span className="text-[10px] bg-amber-500 text-white font-extrabold px-2.5 py-1 rounded-full shadow-sm">
                          Upcoming
                        </span>
                      )}
                      {trip.status === 'Completed' && (
                        <span className="text-[10px] bg-slate-600 text-white font-extrabold px-2.5 py-1 rounded-full shadow-sm">
                          Completed
                        </span>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-xs text-slate-455 font-semibold">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>{trip.memberCount || 1} {trip.memberCount === 1 ? 'Member' : 'Members'}</span>
                        </div>
                      </div>
                    </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-6">
          <div className="bg-white border border-slate-100 p-8 rounded-3xl w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-black text-slate-900 mb-1">Create Trip</h3>
            <p className="text-xs text-slate-455 font-semibold mb-6">Start a new group pool budget for your next adventure</p>
